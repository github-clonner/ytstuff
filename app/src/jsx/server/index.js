...
import { YOUTUBE_SONG, STATE_DOWNLOADED } from '../fetchers/constants';
import { fetchSong, addSong } from '../actions/songs';
...
export default function startServer() {
	...
	io.on('connection', (socket) => {
		...
		socket.on('song', (tab) => {
			...
			store.dispatch(fetchSong({ id: videoId, type: YOUTUBE_SONG }, LIST_START))
				.then(song => download(song))
				.then(song => store.dispatch(addSong(Object.assign({}, song, { state: STATE_DOWNLOADED }), LIST_START)));
		});
		/*socket.on('song', (tab) => {
			const videoId = url.parse(tab.url, true).query['v'];
			if(videoId) {
